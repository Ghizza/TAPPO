...
def force_window_on_top(window):
    """Forza una finestra a rimanere in primo piano, specialmente su Windows"""
    # Porta la finestra in primo piano
    window.lift()
    window.attributes('-topmost', True)
    
    # Dopo un breve delay, rimuovi topmost per permettere interazioni normali
    # ma mantieni la finestra attiva
    window.after(100, lambda: window.attributes('-topmost', False))
    window.after(200, lambda: window.focus_force())
    
    # Su Windows, assicurati che la finestra sia visibile
    if platform.system() == "Windows":
        window.after(300, lambda: window.lift())
...
class InfoWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.after(10, lambda: force_window_on_top(self))
